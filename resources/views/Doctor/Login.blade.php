        ...
        <div class="pt-5">
          <form method = 'post' action = "{{route('doctor.login')}}" style="100%">
            ...
            @foreach ($errors->all() as $error)
              <div class = 'alert alert-warning text-danger' style = "float:center;margin-left:24px;margin-bottom:5px">{{$error}}</div>
            @endforeach
          
          @endif
          @if($message = Session::get('message'))
            <div class ="alert alert-warning text-danger" style = "float:center;margin-left:24px;margin-bottom:5px">
              {{$message}}
            </div>
          @endif
